...
public class BoarScript : MonoBehaviour
{
    ...
    public Enemy1State enemy_State;
    ...
    public float attack_Distance = 2.8f;
   ...
   // static int i;
    ...
    void Start()
    {
        enemy_State = Enemy1State.PATROL;
    ...
    void Update()
    {
        ...
        if (enemy_State == Enemy1State.PATROL)
        {
            Patrol();
        }
        if (enemy_State == Enemy1State.CHASE)
        {
            Chase();
        }
        if (enemy_State == Enemy1State.ATTACK)
        {
    ...
    void Chase()
    {
        ...
        if (Vector3.Distance(transform.position, target.position - new Vector3(0f, target.position.y - this.transform.position.y, 0f)) <= attack_Distance)
        {
            enemy_Anim.Run(false);
            enemy_Anim.Walk(false);
            enemy_State = Enemy1State.ATTACK;
